...
actual class AudioInterface actual constructor() {

    actual constructor(registers: ByteArray) : this() {
    ...
    init {
        ...
        channel1 = SquareWaveGenerator(this@AudioInterface.sampleRateAudio)
        channel2 = SquareWaveGenerator(this@AudioInterface.sampleRateAudio)
        channel3 = VoluntaryWaveGenerator(this@AudioInterface.sampleRateAudio)
        channel4 = NoiseGenerator(this@AudioInterface.sampleRateAudio)
    }
    ...
    actual fun outputSound() {
        if (soundEnabledAudio && speed.output()) {
            soundLine?.let {
                val numSamples: Int = if (this@AudioInterface.sampleRateAudio / 28 >= it.available().times(2)) {
                    it.available() * 2
                } else {
                    (this@AudioInterface.sampleRateAudio / 28) and 0xFFFE
                }

                val b = ByteArray(numSamples)
                if (channel1EnableAudio) channel1.play(b, numSamples / 2, 0)
                if (channel2EnableAudio) channel2.play(b, numSamples / 2, 0)
                if (channel3Enable) channel3.play(b, numSamples / 2, 0)
                if (channel4EnableAudio) channel4.play(b, numSamples / 2, 0)
    ...
    fun initSoundHardware(): SourceDataLine? {
        try {
            val format = AudioFormat(
                ...
                this@AudioInterface.sampleRateAudio.toFloat(), 8, 2, 2, this@AudioInterface.sampleRateAudio.toFloat(), true
            )
            ...
            if (AudioSystem.isLineSupported(lineInfo)) {
                ...
                val bufferLength = (this@AudioInterface.sampleRateAudio / 1000) * bufferLengthMsec
    ...
    fun setSampleRate(sr: Int) {
        this@AudioInterface.sampleRateAudio = sr
    ...
    inner class NoiseGenerator {
        ...
        fun play(b: ByteArray, length: Int, offset: Int) {
            if (totalLength != 0) {
                ...
                val step = ((finalFreq) / (sampleRateNoise shr 8))
